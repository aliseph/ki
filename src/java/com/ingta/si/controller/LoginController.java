...
public class LoginController extends BaseController {
    ...
    public String index(ModelMap model, Integer c) {
        if (c != null) {
            switch (c) {
                case 1:
                    model.addAttribute(DWZResponse.MESSAGE, SpringContextUtil.getMessage("response.captcha.error"));
                    break;
                case 2:
                    model.addAttribute(DWZResponse.MESSAGE, SpringContextUtil.getMessage("response.login.fail"));
                    break;
                case 3:
                    model.addAttribute(DWZResponse.MESSAGE, SpringContextUtil.getMessage("response.user.not.login"));
                    break;
            }
        }
        return "login";

package com.ingta.framework.util.ip;
...
public class IPSeeker {
	...
	// 保存的文件夹
	// private String INSTALL_DIR="e:/qqwry";
	/**
	 * 私有构造函数
	 * 
	...
	private IPSeeker() {
		...
		try {
			...
			String baseClassDir = IPSeeker.class.getResource("QQWry.dat")
					.getPath();
			baseClassDir = URLDecoder.decode(baseClassDir, "UTF-8");
			// String path=baseClassDir+fileName;
			ipFile = new RandomAccessFile(baseClassDir, "r");
		} catch (Exception e) {
	...
	private long locateIP(byte[] ip) {
		...
		for (long i = ipBegin, j = ipEnd; i < j;) {
			...
			readIP(m, b4);
			r = compareIP(ip, b4);
			// log.debug(Util.getIpStringFromBytes(b));
			if (r > 0)
				i = m;
			else if (r < 0) {
				if (m == j) {

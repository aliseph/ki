...
function validateCallback(form) {
	...
	var callbackFn;
	if (callback === 'navTabAjaxDone') {
		callbackFn = navTabAjaxDone;
	} else if (callback === 'dialogAjaxDone') {
		callbackFn = dialogAjaxDone;
	}
	$.ajax({
		...
		dataType : "json",
		cache : false,
		success : callbackFn || DWZ.ajaxDone,
		error : DWZ.ajaxError
	});
}
	...
	ajaxTodo : function() {
		return this.each(function() {
			...
			$this.click(function(event) {
				...
				var title = unescape($this.attr("title")).replaceNameById(
						$(event.target).parents(".unitBox:first"));
				if (title) {
